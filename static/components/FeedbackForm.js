  ...
  renderItem(item, i) {
    return <div className="input-group input-group-sm">
      <input type="text" className="form-control" placeholder="Answer"
        value={item} onChange={e => this.onItemChange(i, e)}/>
      <span className="input-group-btn">
        <button className="btn btn-default" type="button"
          onClick={() => this.onRemove(i)}>Remove</button>
      </span>
    </div>;
  }
  render() {
    ...
    return <div>
      ...
      <div>
        <br/>
        <button type="button"
          className="btn btn-xs btn-default" onClick={this.addItem}>+ Add answer</button>
      </div>
    </div>;
  }
}
