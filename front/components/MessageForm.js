...
export default class MessageForm extends Component {
  ...
  onSubmit(e) {
    ...
    this.props.onAdd({
      ...
      answers: this.state.feedback
        ? this.state.feedback.map(a => ({
            text: a,
          }))
        : [],
    })
  ...
  render() {
    ...
    const feedbackEl =
      feedback == null
        ? <button className="btn btn-link" onClick={() => this.setState({feedback: []})}>
            + Add feedback
          </button>
        : <FeedbackForm items={feedback} onChange={f => this.setState({feedback: f})} />
    ...
    return (
      <form onSubmit={e => this.onSubmit(e)} className="form">
        <div className="form-group">
          ...
          <label>
            <input type="checkbox" checked={useMarkdown} onChange={e => this.onMdChange(e)} /> Use
            markdown
          </label>
          <textarea className="form-control" id="" name="" rows="10" ref={i => (this.text = i)} />
        </div>
        <div className="form-group">
